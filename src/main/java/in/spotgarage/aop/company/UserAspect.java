package in.spotgarage.aop.company;

import in.spotgarage.security.SecurityUtils;
import in.spotgarage.repository.UserRepository;
import in.spotgarage.domain.User;
import in.spotgarage.service.dto.UserDTO;
import in.spotgarage.domain.CompanyParameter;
...
private final Logger log = LoggerFactory.getLogger(UserAspect.class);
     ...
     */
    @Before(value = "execution(* in.spotgarage.service.UserService.createUser(..)) && args(userDTO, ..)")
     ...
     */
    @Before(value = "execution(*  in.spotgarage.service.UserService.updateUser(..)) && args(userDTO, ..)")
     ...
     * Adds tenant information to DTO.
     */
    @Before(value = "execution(* in.spotgarage.repository.UserRepository.save(..)) && args(user, ..)")
     ...
     */
    @Before(value = "execution(* in.spotgarage.service.UserService.getUserWithAuthoritiesByLogin(..)) && args(login, ..)")
     ...
     * Adds filtering to prevent display of information from another tenant
     */
    @Before(value = "execution(* in.spotgarage.service.UserService.getUserWithAuthorities(..)) && args(id, ..)")
